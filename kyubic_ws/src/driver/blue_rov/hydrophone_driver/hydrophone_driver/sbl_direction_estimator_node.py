...
Publishes planner_msgs/PingerDirection for CheckPingerPitch / WriteHydrophoneWaypointCSV.
"""
...
class SblDirectionEstimator(Node):
    ...
    def __init__(self) -> None:
        super().__init__("hydrophone_driver")
        ...
        self.publisher = self.create_publisher(PingerDirection, topic_name, 10)
        try:
        ...
        self.get_logger().info(f"Hydrophone driver ready. Publishing to {topic_name}")
        ...
        for i in range(self.channel_count - 1):
            for j in range(i + 1, self.channel_count):
                ...
                expected_angles.append(
                    np.arcsin(np.clip(self.sound_velocity * expected_delay / baseline, -1, 1))
                )
                observed_angles.append(
                    np.arcsin(
                        np.clip(
                            self.sound_velocity * observed_delays[pair_index] / baseline,
                            -1,
                            1,
                        )
                    )
                ...
                pair_index += 1
        return float(
            np.sum(np.abs(np.asarray(expected_angles) - np.asarray(observed_angles)) * weights)
        )
