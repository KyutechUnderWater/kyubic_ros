...
bool G366::update()
{
  ...
  if (len == 36 && buf[0] == 0x80 && buf[35] == 0x0d) {
    ...
    data.x_gyro = convert_gyro(raw_data_t.x_gyro);
    data.y_gyro = convert_gyro(raw_data_t.y_gyro);
    data.z_gyro = convert_gyro(raw_data_t.z_gyro);
    data.x_accl = convert_accel(raw_data_t.x_accl);
    data.y_accl = convert_accel(raw_data_t.y_accl);
    data.z_accl = convert_accel(raw_data_t.z_accl);
