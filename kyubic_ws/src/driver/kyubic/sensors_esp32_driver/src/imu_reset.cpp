/**
 * @file imu_reset.cpp
 * @brief imu reset driver
 * @author R.Ohnishi
 * @date 2025/11/21
 *
 * @details IMUのハードウェアリセットをSensor基板に送信
 *****************************************************/

#include "sensors_esp32_driver/imu_reset.hpp"
...
ImuReset::ImuReset(const rclcpp::NodeOptions & options) : rclcpp::Node("imu_reset", options)
{
  ...
  try {
    ...
    RCLCPP_INFO(this->get_logger(), "IMU reset open port: %d", this_port);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(this->get_logger(), "Failed to initialize udp publisher: %s", e.what());
    RCLCPP_ERROR(this->get_logger(), "IMU reset don't open port %d", this_port);
    exit(1);
  }
  ...
  sub_ = this->create_subscription<driver_msgs::msg::BoolStamped>(
    "imu_reset", 10, [this](const driver_msgs::msg::BoolStamped & _msg) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(driver::sensors_esp32_driver::ImuReset)
