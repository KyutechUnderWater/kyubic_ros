...
#include "sensor_board_driver/imu_reset.hpp"

namespace sensor_board_driver
{

ImuReset::ImuReset() : rclcpp::Node("imu_reset")
{
  ...
  sub_ = this->create_subscription<driver_msgs::msg::BoolStamped>(
    "imu_reset", 10, [this](const driver_msgs::msg::BoolStamped::SharedPtr _msg) {
...
int main(int argc, char * argv[])
{
  ...
  try {
    auto node = std::make_shared<sensor_board_driver::ImuReset>();
    rclcpp::spin(node);
