...
#include "sensor_board_driver/zed_power.hpp"

namespace sensor_board_driver
{

ZedPower::ZedPower() : rclcpp::Node("zed_power")
{
  ...
  sub_ = this->create_subscription<driver_msgs::msg::BoolStamped>(
    "zed_power", 10, [this](const driver_msgs::msg::BoolStamped::SharedPtr _msg) {
...
int main(int argc, char * argv[])
{
  ...
  try {
    auto node = std::make_shared<sensor_board_driver::ZedPower>();
    rclcpp::spin(node);
