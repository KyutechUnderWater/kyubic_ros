...
#include "sensor_board_driver/rtc_gnss.hpp"
...
RtcGnss::RtcGnss() : Node("rtc_gnss")
{
  ...
  pub_ = create_publisher<driver_msgs::msg::RtcGnss>("rtc_gnss", qos);
  ...
  protolink_subscriber_ = std::make_shared<protolink::udp_protocol::Subscriber<ProtoRtcGnss>>(
    io_context_, sub_port, [this](const auto & _msg) {
      ...
      driver_msgs::msg::RtcGnss msg = convert(_msg);
      msg.header.stamp = this->get_clock()->now();
      msg.header.frame_id = "gnss_link";

      pub_->publish(msg);
      RCLCPP_DEBUG(
        this->get_logger(), "Publish ---> Lat: %.6f, Lon: %.6f, Alt: %.2f, Sat: %d", msg.latitude,
        msg.longitude, msg.altitude, msg.satellites);
    });

  RCLCPP_INFO(this->get_logger(), "RTC GNSS Driver Connected Port: %d", sub_port);

  timer_ = create_wall_timer(100ms, std::bind(&RtcGnss::_check_timeout, this));
}

void RtcGnss::_check_timeout()
{
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (timeout_->check(this->get_clock()->now())) {
    auto msg = std::make_unique<driver_msgs::msg::RtcGnss>();
    msg->header.stamp = this->get_clock()->now();
    msg->status.id = common_msgs::msg::Status::ERROR;

    pub_->publish(std::move(msg));
    RCLCPP_ERROR(this->get_logger(), "RTC GNSS timeout: %lu [ns]", timeout_->get_elapsed_time());
  } else {
    RCLCPP_WARN(this->get_logger(), "Faild to get rtc-gnss data");
    return;
  }
}
...
int main(int argc, char ** argv)
{
  ...
  try {
    auto node = std::make_shared<sensor_board_driver::RtcGnss>();
    rclcpp::spin(node);
