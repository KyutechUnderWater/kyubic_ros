...
class RtcTimeTopicStatusCheck : public system_health_check::base::TopicStatusCheckBase<Msg>
{
private:
  void prepare_check(rclcpp::Node::SharedPtr node) override
  {
    std::string topic_name = node->declare_parameter(
      "driver.sensors_esp32_driver.rtc_time_topic_status_check.topic_name", "/rtc_time");
    uint32_t timeout_ms = node->declare_parameter(
      "driver.sensors_esp32_driver.rtc_time_topic_status_check.timeout_ms", 1000);

    set_status_id("rtc_time_status");
...
PLUGINLIB_EXPORT_CLASS(
  driver::sensors_esp32_driver::RtcTimeTopicStatusCheck, system_health_check::base::SystemCheckBase)
