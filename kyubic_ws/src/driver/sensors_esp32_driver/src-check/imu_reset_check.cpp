...
class ImuResetSubscriberCheck : public system_health_check::TopicSubscriberCheckBase
{
public:
  bool check(rclcpp::Node::SharedPtr node) override
  {
    std::string topic_name = node->declare_parameter(
      "sensors_esp32_driver.imu_reset_subscriber_check.topic_name", "/imu_reset");
    uint32_t timeout_ms =
      node->declare_parameter("sensors_esp32_driver.imu_reset_subscriber_check.timeout_ms", 1000);
...
PLUGINLIB_EXPORT_CLASS(
  sensors_esp32_driver::ImuResetSubscriberCheck, system_health_check::SystemCheckBase)
