...
class SystemStatusTopicStatusCheck : public system_health_check::base::TopicStatusCheckBase<Msg>
{
private:
  void prepare_check(rclcpp::Node::SharedPtr node) override
  {
    std::string topic_name = node->declare_parameter(
      "driver.sensors_esp32_driver.system_status_topic_status_check.topic_name", "/system_status");
    uint32_t timeout_ms = node->declare_parameter(
      "driver.sensors_esp32_driver.system_status_topic_status_check.timeout_ms", 1000);

    set_status_id("system_status_status");
