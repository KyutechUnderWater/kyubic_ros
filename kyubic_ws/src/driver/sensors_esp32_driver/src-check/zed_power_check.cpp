...
class ZedPowerSubscriberCheck : public system_health_check::base::TopicSubscriberCheckBase
{
private:
  void prepare_check(rclcpp::Node::SharedPtr node) override
  {
    std::string topic_name = node->declare_parameter(
      "driver.sensors_esp32_driver.zed_power_subscriber_check.topic_name", "/zed_power");
    uint32_t timeout_ms = node->declare_parameter(
      "driver.sensors_esp32_driver.zed_power_subscriber_check.timeout_ms", 1000);
...
PLUGINLIB_EXPORT_CLASS(
  driver::sensors_esp32_driver::ZedPowerSubscriberCheck, system_health_check::base::SystemCheckBase)
