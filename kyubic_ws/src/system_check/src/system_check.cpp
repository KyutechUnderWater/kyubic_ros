...
#include "system_check/system_check.hpp"
...
SystemCheck::SystemCheck(const rclcpp::NodeOptions & options) : Node("checker_node", options)
{
  ...
  try {
    loader_ = std::make_shared<pluginlib::ClassLoader<system_check::SystemCheckBase>>(
      "system_check", "system_check::SystemCheckBase");
  } catch (pluginlib::PluginlibException & ex) {
    RCLCPP_ERROR(this->get_logger(), "Failed to create ClassLoader: %s", ex.what());
  }

  init_timer_ = this->create_wall_timer(100ms, std::bind(&SystemCheck::run_checks, this));
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(system_check::SystemCheck)
