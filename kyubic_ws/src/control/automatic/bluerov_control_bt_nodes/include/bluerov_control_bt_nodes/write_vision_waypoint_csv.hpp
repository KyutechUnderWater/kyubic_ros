...
/**
 * @brief buoy_interfaces/msg/BuoyRelativePositionとodomから、ブイを画面中心へ寄せながら
 * 前進する1点をpath_planner形式のCSVへ書き出すBT葉ノード(iwakuni2026.xml、
 * `CenteringStep`)。`WriteHydrophoneWaypointCSV`と構造は同じ(odom/検出を待つ→計算→
 * waypoint_csv_writerでatomic write)だが、入力メッセージ型と、水平/垂直で別の情報源から
 * 補正量を作る点が異なる。
 *
 * @details 計算の流れ:
 * 1. confidenceが`confidence_threshold`未満、または`detected==false`ならこのtickは
 *    何もせずFAILURE(RetryUntilSuccessfulが次tickで再試行する。安全側)。
 * 2. yaw補正: `yaw_valid`なら`relative_buoy_yaw_rad`をそのまま使う。そうでなければ
 ...
 * 5. z_step: `position_valid`なら`relative_buoy_z_m × vertical_gain_m`
 *    (【未検証・要実機確認】`relative_buoy_z_m`は機体座標系で正=下方向、NED慣習と
 *    一致する前提。`vertical_gain_m`はここでは無次元の比例ゲインとして使う
 *    ("1tickでこの割合だけ縦方向の推定誤差に近づく"という意味。仕様書は元々
 *    `normalized_vertical_offset`(無次元、画像処理班が未実装)を使う想定だったが、
 *    そのフィールドが`BuoyRelativePosition.msg`に存在しないため、依頼者と合意の上
 *    `relative_buoy_z_m`で代用した)。`position_valid=false`の間はz_step=0
 *    (垂直方向の補正はせず、水平方向の中心合わせのみ進める)。
 ...
 *
 * odom/buoy_relative_positionのどちらかを一度も受信していない間はRUNNINGのまま待機する
 * (WriteHydrophoneWaypointCSV等と同じ安全側の設計)。
 */
