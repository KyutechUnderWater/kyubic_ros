...
void Serial::setConfig(const int _baudrate) {
  ...
  default:
    std::string s =
        std::string("Unknown baud rate ") + std::to_string(_baudrate);
    throw SerialException(s);
  }
...
ssize_t
Serial::read(uint8_t *buf, const size_t len,
             const std::chrono::duration<long, std::ratio<1, 1000>> timeout) {
  bool end_flag = false;
  std::future<ssize_t> future =
      std::async(std::launch::async, [this, buf, &len, &end_flag]() {
        ssize_t read_len = 0;
        while (!end_flag) {
...
ssize_t
Serial::read_until(uint8_t *buf, const size_t len, const char end_char,
